...
void OBJLoader::createFaceNormals()
{
    for (int i = 0; i < _faces.size(); ++i)
    {
        ...
        vmml::Vector3f e1 = p2 - p1;
        vmml::Vector3f e2 = p3 - p1;
        
        // calculate normal for this face
        vmml::Vector3f normal = e1.cross(e2);
        
        // set face normal
        face.normal = vmml::normalize(normal);
        
        //the delta UV coordinates
        vmml::Vector2f U = t2 - t1;
        vmml::Vector2f V = t3 - t1;
        
        float tmp = 1.0f / (U.x() * V.y() - V.x() * U.y());
        float x = (tmp * (V.y() * e1.x() - V.y() * e2.x()));
        float y = (tmp * (V.y() * e1.y() - U.y() * e2.y()));
        float z = (tmp * (V.y() * e1.z() - U.y() * e2.z()));
        face.tangent = normalize(vmml::Vector3f(x,y,z));
